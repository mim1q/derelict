...
class FancyCobwebWithSpiderNestBlock(settings: Settings) : FancyCobwebBlock(settings) {
  private fun spawnParticle(state: BlockState, world: WorldAccess, blockPos: BlockPos, random: Random, chance: Float) {
  ...
  override fun randomDisplayTick(state: BlockState, world: World, pos: BlockPos, random: Random) {
    spawnParticle(state, world, pos, random, 0.25F)
  }
  ...
  override fun onEntityCollision(state: BlockState, world: World, pos: BlockPos, entity: Entity) {
    if (world.isClient) {
      repeat(2) {
        spawnParticle(state, world, pos, world.random, 0.5F)
      }
    }
  ...
  override fun onBroken(world: WorldAccess, pos: BlockPos, state: BlockState) {
    ...
    repeat(40) {
      spawnParticle(state, world, pos, world.random, 1.0F)
    }
  }
}